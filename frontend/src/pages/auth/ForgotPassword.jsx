...
// //               placeholder="you@gau.ac.ke or E101/1234/23"
// //             />
// //           </div>
  ...
  async function handleSubmit(e) {
    ...
    if (!email.trim()) {
      return setStatus({
        loading: false,
        ok: false,
        msg: "Please enter your email or reg number.",
      });
    }
    setStatus({ loading: true, ok: null, msg: "" });

    try {
      // ✅ use api.post (your wrapper automatically adds BASE + headers)
      const res = await api.post("/auth/forgot", { email });

      setStatus({
        loading: false,
        ok: true,
        msg: res.message || "If this account exists, a reset link was sent.",
      });
    } catch (err) {
      setStatus({
        loading: false,
