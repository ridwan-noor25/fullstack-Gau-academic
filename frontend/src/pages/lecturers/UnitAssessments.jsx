...
//         // 2) Normalize students → ensure reg_number is included
//         const normalizedStudents = (Array.isArray(studs) ? studs : [])
//           .map((e) => (e && typeof e === "object" && "student" in e ? e.student : e))
//           .filter(Boolean)
//           .map((s) => ({
//             id: s.id,
//             name: s.name || "Unnamed",
//             reg_number: s.reg_number || "—",   // ✅ ensure reg no is present
//           }));
//         setStudents(normalizedStudents);
...
//                   <tr>
//                     <Th>Student</Th>
//                     <Th>Reg No</Th> {/* ✅ Added Reg No column */}
//                     <Th>Score</Th>
//                   </tr>
//                 </thead>
//                 <tbody className="divide-y">
...
//                     <tr key={s.id} className="hover:bg-gray-50">
//                       <Td className="font-medium">{s.name}</Td>
//                       <Td>{s.reg_number}</Td> {/* ✅ Now visible */}
//                       <Td>
//                         <input
//                           type="number"
//                           className="w-28 rounded-md border border-gray-300 p-1 focus:ring-green-600 focus:border-green-600"
...
//                   <Th>Due</Th>
//                   <Th>Published</Th>
//                   <Th className="w-48">Actions</Th> {/* ✅ reserve space for buttons */}
//                 </tr>
//               </thead>
//               <tbody className="divide-y">
//                 {items.length === 0 && (
...
//                     </Td>
//                     <Td>
//                       <div className="flex flex-wrap gap-2"> {/* ✅ flex-wrap prevents squeezing */}
//                         <button
//                           onClick={() => openGrades(a)}
//                           className="text-xs px-3 py-1.5 rounded-md border border-green-700 text-green-700 hover:bg-gray-50"
//                         >
...
//   return (
//     <th className={`px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 ${className}`}>
//       {children}
//     </th>
//   );
// }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const normalizedStudents = (Array.isArray(studs) ? studs : [])
          .map((e) =>
            e && typeof e === "object" && "student" in e ? e.student : e
          )
          .filter(Boolean)
          .map((s) => ({
            id: s.id,
        ...
        const map = {};
        (Array.isArray(gradesPack) ? gradesPack : []).forEach(
          ({ assessment, grades }) => {
            if (!assessment || !assessment.id) return;
            const aid = assessment.id;
            map[aid] = {};
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <Th>Due</Th>
                <Th>Published</Th>
                <Th className="w-60">Actions</Th> {/* ✅ Wider column */}
              </tr>
            </thead>
            <tbody className="divide-y">
              {items.length === 0 && (
