...
const StatCard = ({ label, value, Icon, tone = "green", hint }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm ring-1 ${tones.ring} p-4 sm:p-5`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs sm:text-sm text-gray-500">{label}</p>
          <p className="mt-1 text-xl sm:text-2xl font-bold text-gray-900">{value}</p>
          {hint ? <p className="mt-1 text-[11px] sm:text-xs text-gray-500">{hint}</p> : null}
        </div>
        {Icon ? <Icon className={`h-6 w-6 sm:h-8 sm:w-8 ${tones.icon}`} /> : null}
      </div>
    </div>
  );
};
...
const Pill = ({ children, tone = "gray" }) => {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] sm:text-xs ${tones}`}>
      {children}
    </span>
  );
};
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 lg:px-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        <div>
          {/* <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-green-900">Admin Dashboard</h1> */}
          <p className="text-sm text-gray-600">
            Manage departments, users, units, and system health at a glance.
          </p>
          {err && (
            <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              {err}
        ...
        {/* <button
          onClick={fetchData}
          className="inline-flex w-full sm:w-auto justify-center items-center gap-2 rounded-lg bg-white border px-3 py-2 text-sm hover:bg-gray-50"
          title="Refresh"
        >
          <ArrowPathIcon className="h-5 w-5" />
          Refresh
      ...
      {loading ? (
        ...
        <>
          {/* KPIs */}
          <div className="mt-5 sm:mt-6 grid grid-cols-1 gap-3 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <StatCard label="Total Users" value={stats.totalUsers} Icon={UsersIcon} tone="blue" />
            <StatCard label="Students" value={stats.students} Icon={UsersIcon} tone="green" />
            <StatCard label="Lecturers" value={stats.lecturers} Icon={UsersIcon} tone="green" />
            <StatCard label="HoDs" value={stats.hods} Icon={UsersIcon} tone="green" />
          ...
          {/* Recent + Snapshot */}
          <div className="mt-6 lg:mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
            {/* Recent Activity */}
            <section className="lg:col-span-2 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 sm:px-5 py-3 sm:py-4">
                <h2 className="text-base sm:text-lg font-semibold text-gray-900">Recent Activity</h2>
                {Boolean(ADMIN_RECENT_URL) && (
                  <span className="text-xs text-gray-500">
                    Last {Math.min(10, recent.length)} events
                  </span>
                )}
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-[700px] sm:min-w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {recent.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="px-4 sm:px-5 py-6 text-center text-gray-500">
                          No activity yet.
                        </td>
                      </tr>
                    ) : (
                      recent.map((r) => (
                        <tr key={r.id} className="hover:bg-gray-50">
                          ...
                          <Td className="font-medium max-w-[320px] sm:max-w-none truncate">
                            {r.title}
                          </Td>
                          <Td className="hidden md:table-cell">{r.actor}</Td>
                          <Td className="whitespace-nowrap">
                            {new Date(r.at).toLocaleString()}
                          </Td>
                          <Td className="hidden sm:table-cell text-xs text-gray-600">
                            {r.meta?.unit
                              ? `Unit: ${r.meta.unit}`
                              : r.meta?.department
                              ? `Dept: ${r.meta.department}`
                              : "—"}
                          </Td>
                        </tr>
                      ))
            ...
            {/* System Snapshot */}
            <section className="rounded-2xl bg-white p-4 sm:p-5 shadow-sm ring-1 ring-gray-200">
              <div className="flex items-center justify-between">
...
function Th({ children, className = "" }) {
  return (
    <th
      className={`px-4 sm:px-5 py-2.5 sm:py-3 text-[11px] sm:text-xs font-medium uppercase tracking-wider text-gray-500 ${className}`}
    >
      {children}
    </th>
  );
...
function Td({ children, className = "" }) {
  return (
    <td className={`px-4 sm:px-5 py-2.5 sm:py-3 text-sm text-gray-900 ${className}`}>
      {children}
    </td>
  );
}
