...
#     # Accepts camelCase keys from your frontend: name, regNumber, course, email, password
#     data = request.get_json(silent=True) or {}

#     required = ["name", "regNumber", "course", "email", "password"]
#     missing = [k for k in required if not data.get(k)]
#     if missing:
#         return {"error": f"Missing fields: {', '.join(missing)}"}, 400

#     name = data["name"].strip()
#     reg_number = data["regNumber"].strip()
...
#         return {"error": "Registration number already in use."}, 409

#     # Create & persist
#     student = Student(name=name, reg_number=reg_number, course=course, email=email)
...
# def login_student():
#     # Accepts camelCase keys from your frontend: regNumber, password
#     data = request.get_json(silent=True) or {}
#     reg_number = (data.get("regNumber") or "").strip()
#     password = data.get("password") or ""

#     if not reg_number or not password:
#         return {"error": "Please provide regNumber and password."}, 400
...
#         "student": student.to_dict()  # safe—no password hash
#     }, 200
